...
class CouponService {
    ...
    static async createCoupon({
        ...
        const couponPayload = new CouponBuilder()
            ...
            .withAssignedUsers(assigned_users)
            .build()
    ...
    // Lấy coupon theo code - CHỈ CHO PHÉP PUBLIC HOẶC ASSIGNED USER
    static async getCouponByCode(code, userId = null) {
        const coupon = await CouponRepository.findByCode(code, {
            populate: [
                { path: 'applicable_categories', select: 'name slug' },
                { path: 'applicable_products', select: 'name slug' },
                { path: 'assigned_users', select: 'email name' }, // Populate assigned users
            ],
        })
        ...
        // Security check: Nếu là private coupon
        if (coupon.type === 'private') {
            if (!userId) {
                throw new BadRequestError('Authentication required for private coupon')
            }
            
            // Nếu có assigned_users thì check, nếu không thì cho phép tất cả user đăng nhập
            if (coupon.assigned_users && coupon.assigned_users.length > 0) {
                const isAssigned = coupon.assigned_users.some(
                    user => user._id.toString() === userId.toString()
                )
                
                if (!isAssigned) {
                    throw new BadRequestError('This coupon is not available for you')
                }
            }
        }
    ...
    // Lấy coupons còn hiệu lực - CHỈ HIỆN PUBLIC + FEATURED/LANDING_PAGE
    static async getActiveCoupons({ page = 1, limit = 10, visibility = null }) {
        const skip = (page - 1) * limit
        
        // Filter: chỉ public coupons và có visibility phù hợp
        const additionalFilter = {
            type: 'public',  // Chỉ public coupons
        }
        
        // Nếu có specify visibility thì filter theo đó
        if (visibility) {
            additionalFilter.visibility = visibility
        } else {
            // Mặc định chỉ hiện featured và landing_page (không hiện hidden)
            additionalFilter.visibility = { $in: ['featured', 'landing_page'] }
        }
    ...
    static async validateCoupon({
        ...
        // 1. Tìm coupon
        const coupon = await CouponRepository.findByCode(code, {
            populate: [{ path: 'assigned_users', select: 'email name' }]
        })
        if (!coupon) {
            throw new NotFoundError('Mã giảm giá không tồn tại')
        }

        // 2. Security check: Nếu là private coupon
        if (coupon.type === 'private') {
            if (!userId) {
                throw new BadRequestError('Authentication required for private coupon')
            }
        ...
        // 8. Kiểm tra applicable categories
        ...
        // 9. Kiểm tra applicable products
    ...
    // Lấy coupons áp dụng cho category - CHỈ PUBLIC
    static async getCouponsByCategory(categoryId, { page = 1, limit = 10 }) {
        const skip = (page - 1) * limit
        
        // Filter: chỉ public coupons
        const additionalFilter = {
            type: 'public',
            visibility: { $in: ['featured', 'landing_page'] }  // Chỉ hiện public + visible
        }
        
        const coupons = await CouponRepository.findByCategory(categoryId, { 
            skip, 
            limit, 
            additionalFilter 
        })
    ...
    // Lấy coupons áp dụng cho product - CHỈ PUBLIC  
    static async getCouponsByProduct(productId, { page = 1, limit = 10 }) {
        ...
        const coupons = await CouponRepository.findByProduct(productId, { 
            skip, 
            limit, 
            additionalFilter 
        })
